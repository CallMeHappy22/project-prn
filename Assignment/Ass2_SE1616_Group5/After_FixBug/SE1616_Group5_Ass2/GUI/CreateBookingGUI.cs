...
namespace Ass2_SE1616_Group5.GUI
{
    public partial class CreateBookingGUI : Form
    {
        ...
        {
            List<Booking> listBooking = cinemaContext.Bookings.AsEnumerable().Where(b => b.ShowId == showID).ToList();
            StringBuilder result = new StringBuilder();

            if (listBooking.Count <= 0) return result.ToString();
            ...
            for (int i = 0; i < listBooking[0].SeatStatus.Length; i++)
            {
                ...
                if (check) result.Append('1'); else result.Append('0');
            }
